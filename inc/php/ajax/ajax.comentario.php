...
$psPage = 'php_files/p.comentario.' . $niveles[$action]['p'];
...
switch($action){
	...
	case 'comentario-ajax':
		...
		$psComentario = $psPosts->getComentarios($psPost);
		$psComentarios = array(
			'num' => $psComentario['num'],
			'data' => $psComentario['data'],
			'block' => $psComentario['block'],
			'autor' => $psAutor
		);
